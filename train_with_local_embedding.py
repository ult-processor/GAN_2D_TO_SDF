...
from utils import *
from model_with_local import * 
from config import cfg


sdf_directory = './logs_with_local_flops/train_samples/org/'
ply_directory = './logs_with_local_flops/train_samples/ply/'
sp_directory = './logs_with_local_flops/train_samples/sp/'
sz_directory = './logs_with_local_flops/train_samples/sz/'
ckpt_directory = './logs_with_local_flops/checkpoints/'

input_dir = os.path.join(sys.path[0], 'data/')
train_file = os.path.join(input_dir, "train_withLabel_s64p1024.h5")
test_file = os.path.join(input_dir,"test_withLabel_s64p1024.h5")
# train_file = os.path.join(input_dir, "chair_s64p1024_train.h5")
...
def trainGAN():
    ...
    from_pointnet_ph = tf.placeholder(tf.bool, shape=())
    ...
    #-------------------Building a model-------------------------#
    global_mean, global_log_sigma, local_mean, local_log_sigma,\
    s_p, s_p_sigmoid, s_p_nosig, s_z, s_z_nosig, s_z_sigmoid,\
    s_sigmoid, s_nosig = build_model(p_vector, s_vector, z_vector)

    #-------------------END of Building a model-------------------------#
    d_real_loss, d_fake_loss, d_encode_loss, d_loss,\
    rec_loss, local_kl_loss, glob_kl_loss, total_kl_loss,\
    e_loss, g_fake_loss, g_encode_loss, g_loss = vae_gan_loss(global_mean, global_log_sigma, local_mean, local_log_sigma,\
                                                              s_vector, s_p, s_p_nosig, s_z_nosig, s_nosig)
    ...
    g_vars = [var for var in t_vars if any(name in var.name for name in ['local_gen', 'gl_', 'glob_gen', 'gg_'])]
    ...
    printout(flog,'************ Total number of Trainable Parameters: {} ************'.format(num_params))
    num_g_params = np.sum([np.prod(v.get_shape().as_list()) for v in tf.global_variables()])
    printout(flog, '************ The Number of Global Parameters: {} ************'.format(num_g_params))
    num_params = np.sum([np.prod(var.get_shape().as_list()) for var in t_vars if any(name in var.name for name in ['disc', 'd_'])])
    printout(flog,'************ Discriminator number of Trainable Parameters: {} ************'.format(num_params))
    num_params = np.sum([np.prod(var.get_shape().as_list()) for var in t_vars if any(name in var.name for name in ['local_gen', 'gl_', 'glob_gen', 'gg_'])])
    printout(flog,'************ Generator number of Trainable Parameters: {} ************'.format(num_params))
    num_params = np.sum([np.prod(var.get_shape().as_list()) for var in t_vars if any(name in var.name for name in ['encoder', 'e_', 'Tnet', 't_', 'FTnet', 'ft_'])])
    printout(flog,'************ Encoder number of Trainable Parameters: {} ************'.format(num_params))
    ...
    # Embedding
    local_mean_embed = tf.Variable(tf.zeros([448, cfg.z_size]), name='local_mean_embedding')
    glob_mean_embed = tf.Variable(tf.zeros([448, cfg.z_size]), name='glob_mean_embedding')
    local_sigma_embed = tf.Variable(tf.zeros([448, cfg.z_size]), name='local_sigma_embedding')
    glob_sigma_embed = tf.Variable(tf.zeros([448, cfg.z_size]), name='glob_sigma_embedding')
    ...
    with tf.Session(config=config) as sess:
        ...
        # embedding_config.tensor_name = gl_enc_mean_embed.name
        # embedding_config.metadata_path = os.path.join(ckpt_directory, 'test_metadata.tsv')
        # embedding_config.metadata_path = 'test_metadata.tsv'
        #-------------------------------------------------------
        embedding_config = config.embeddings.add()
        embedding_config.tensor_name = local_mean_embed.name
        embedding_config.metadata_path = 'test_metadata.tsv'

        embedding_config = config.embeddings.add()
        embedding_config.tensor_name = glob_mean_embed.name
        embedding_config.metadata_path = 'test_metadata.tsv'

        embedding_config = config.embeddings.add()
        embedding_config.tensor_name = local_sigma_embed.name
        embedding_config.metadata_path = 'test_metadata.tsv'

        embedding_config = config.embeddings.add()
        embedding_config.tensor_name = glob_sigma_embed.name
        embedding_config.metadata_path = 'test_metadata.tsv'
        ...
        for epoch in range(cfg.n_epochs):
            ...
            for step in range(num_batch):
                ...
                locKLoss, globKLoss, totalKLoss, dS, dGsz, dGsp, d_acc = sess.run([d_real_loss, d_fake_loss, d_encode_loss, d_loss,\
                                                                                    g_fake_loss, g_encode_loss, g_loss, e_loss, rec_loss, local_kl_loss,\
                ...
                if total_steps % 10 == 0:
                    ...
                    printout(flog, "\t----------------------------")
                    printout(flog, "\tlocal KL loss: %f" % (local_kl_avg_loss/10))
            ...
            if epoch % 20 == 0:
                # temp_em = np.zeros((448, cfg.z_size), dtype='float32')
                temp_local_mean = np.zeros((448, cfg.z_size), dtype='float32')
                temp_glob_mean = np.zeros((448, cfg.z_size), dtype='float32')
                temp_local_sigma = np.zeros((448, cfg.z_size), dtype='float32')
                temp_glob_sigma = np.zeros((448, cfg.z_size), dtype='float32')
                
                sdf_test, pc_test, label_test = load_sdfplylabel_h5(test_file)
                sdf_test = sdf_test.reshape((-1, cfg.cube_len, cfg.cube_len, cfg.cube_len, cfg.cube_channel))
                
                num_test_batch = len(sdf_test) // cfg.batch_size
                for step in range(num_test_batch):
                    begIdx = step * cfg.batch_size
                    endIdx = begIdx + cfg.batch_size
            
                    # Sample from uniform distribution
                    z_test = np.random.uniform(low=-1.0, high=1.0, size=[cfg.batch_size, cfg.z_size]).astype(np.float32)
                    # temp_em[begIdx:endIdx,...] = sess.run(gl_enc_mean,
                    #                                     feed_dict = {
                    #                                             is_training_ph: False,
                    #                                             p_vector:pc_test[begIdx:endIdx, ...],
                    #                                             s_vector:sdf_test[begIdx:endIdx, ...],
                    #                                             z_vector:z_test
                    #                                                 })
                    # print(temp_em.shape)
                    #-----------------------------
                    temp_local_mean[begIdx:endIdx,...], temp_glob_mean[begIdx:endIdx,...],\
                    temp_local_sigma[begIdx:endIdx,...], temp_glob_sigma[begIdx:endIdx,...],= sess.run([local_mean, global_mean, local_log_sigma, global_log_sigma,], 
                                                                                                        feed_dict = {
                                                                                                                is_training_ph: False,
                                                                                                                p_vector:pc_test[begIdx:endIdx, ...],
                                                                                                                s_vector:sdf_test[begIdx:endIdx, ...],
                ...
                # sess.run(tf.assign(gl_enc_mean_embed, temp_em))
                sess.run(tf.assign(local_mean_embed, temp_local_mean))
                sess.run(tf.assign(glob_mean_embed, temp_glob_mean))
                sess.run(tf.assign(local_sigma_embed, temp_local_sigma))
                sess.run(tf.assign(glob_sigma_embed, temp_glob_sigma))

            opts = tf.profiler.ProfileOptionBuilder.float_operation()    
            flops = tf.profiler.profile(tf.get_default_graph(), run_meta=run_meta, cmd='op', options=opts)
            if flops is not None:
                printout(flog, 'Flops: {}'.format(flops.total_float_ops))
