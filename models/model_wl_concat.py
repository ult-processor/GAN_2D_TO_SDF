...
def point_net_encoder(p_vector, is_training, reuse=False):
    ...
    with tf.variable_scope('encoder', reuse=reuse) as scope:
        ...
        local_feat = tf.nn.relu( \
                            batch_normal(\
                                conv2d(local_feat, output_dim=8, k_h=1, k_w=1, padding='VALID', name='e_lc1'),\
...
def local_generator(local_z, is_training, reuse=False):
    ...
    with tf.variable_scope('local_gen', reuse=reuse) as scope:
        ...
        gen1 = tf.nn.relu(batch_normal(\
                                deconv3d(local_z, output_shape=[4,4,4,256], kernel=[4,4,4], strides=[1,1,1,1], padding='VALID', name='gl_decov1'),\
                                is_training=is_training, name='gl_bn1'))
        # activation_summary(gen1)
        gen2 = tf.nn.relu(batch_normal(\
                                deconv3d(gen1, output_shape=[8,8,8,128], kernel=[5,5,5], strides=strides, name='gl_decov2'),\
                                is_training=is_training, name='gl_bn2'))
        # activation_summary(gen2)
        gen3 = tf.nn.relu(batch_normal(\
                                deconv3d(gen2, output_shape=[16,16,16,64], kernel=[5,5,5], strides=strides, name='gl_decov3'),\
                                is_training=is_training, name='gl_bn3'))
        # activation_summary(gen3)
        gen4 = tf.nn.relu(batch_normal(\
                                deconv3d(gen3, output_shape=[32,32,32,32], kernel=[5,5,5], strides=strides, name='gl_decov4'),\
                                is_training=is_training, name='gl_bn4'))
        # activation_summary(gen4)
        return gen1, gen2, gen3, gen4

def global_generator(glob_z, loc4=None, loc8=None, loc16=None, loc32=None, from_pointnet=True, is_training=False, reuse=False):
    strides = [2,2,2,1]
    with tf.variable_scope('glob_gen', reuse=reuse) as scope:
        ...
        if from_pointnet:
            ...
            gen1 = batch_normal(\
                                    deconv3d(glob_z, output_shape=[4,4,4,256], kernel=[4,4,4], strides=[1,1,1,1], padding='VALID', name='gg_decov1'),\
                                    is_training=is_training, name='gg_bn1')
            # gen1 = tf.nn.relu(tf.multiply(gen1, loc4))
            gen1 = tf.concat([gen1,loc4], axis=-1)
            print(gen1)
            # activation_summary(gen1)
            gen2 = batch_normal(\
                                    deconv3d(gen1, output_shape=[8,8,8,128], kernel=[5,5,5], strides=strides, name='gg_decov2'),\
                                    is_training=is_training, name='gg_bn2')
            # gen2 = tf.nn.relu(tf.multiply(gen2, loc8))
            gen2 = tf.concat([gen2,loc8], axis=-1)
            # activation_summary(gen2)
            print(gen2)
            gen3 = batch_normal(\
                                    deconv3d(gen2, output_shape=[16,16,16,64], kernel=[5,5,5], strides=strides, name='gg_decov3'),\
                                    is_training=is_training, name='gg_bn3')
            # gen3 = tf.nn.relu(tf.multiply(gen3, loc16))
            gen3 = tf.concat([gen3,loc16], axis=-1)
            print(gen3)
            # activation_summary(gen3)
            gen4 = batch_normal(\
                                    deconv3d(gen3, output_shape=[32,32,32,32], kernel=[5,5,5], strides=strides, name='gg_decov4'),\
                                    is_training=is_training, name='gg_bn4')
            # gen4 = tf.nn.relu(tf.multiply(gen4, loc32))
            gen4 = tf.concat([gen4,loc32], axis=-1)
            print(gen4)
            # activation_summary(gen4)
            gen5 = deconv3d(gen4, output_shape=[64,64,64,1], kernel=[5,5,5], strides=strides, name='gg_decov5')
            ...
            gen1 = tf.nn.relu(batch_normal(\
                                    ...
                                    is_training=is_training, name='gg_bn1'))
            # activation_summary(gen1)
            gen1 = tf.concat([gen1, gen1], axis=-1)
            gen2 = tf.nn.relu(batch_normal(\
                                    deconv3d(gen1, output_shape=[8,8,8,128], kernel=[5,5,5], strides=strides, name='gg_decov2'),\
                                    is_training=is_training, name='gg_bn2'))
            # activation_summary(gen2)
            gen2 = tf.concat([gen2, gen2], axis=-1)
            gen3 = tf.nn.relu(batch_normal(\
                                    deconv3d(gen2, output_shape=[16,16,16,64], kernel=[5,5,5], strides=strides, name='gg_decov3'),\
                                    is_training=is_training, name='gg_bn3'))
            # activation_summary(gen3)
            gen3 = tf.concat([gen3, gen3], axis=-1)
            gen4 = tf.nn.relu(batch_normal(\
                                    deconv3d(gen3, output_shape=[32,32,32,32], kernel=[5,5,5], strides=strides, name='gg_decov4'),\
                                    is_training=is_training, name='gg_bn4'))
            # activation_summary(gen4)
            gen4 = tf.concat([gen4, gen4], axis=-1)
            gen5 = deconv3d(gen4, output_shape=[64,64,64,1], kernel=[5,5,5], strides=strides, name='gg_decov5')
...
def build_model(p_vector, s_vector, z_vector):
    ...
    loc4, loc8, loc16, loc32 = local_generator(local_z, is_training=True, reuse=False)
    s_p = global_generator(glob_z, loc4, loc8, loc16, loc32, is_training=True, reuse=False)
